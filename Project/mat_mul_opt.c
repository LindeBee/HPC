...
int main(int argc, char* argv[])
{
    ...
    if ((m_dim/p_dim)%2 != 0){ m_dim += p_dim;} //make sure sub_dim is even
    int *full_A;
    int *full_B;
    int *full_C;
    int *ser_C;
    ...
    // find STRIP length
    int STRIP; //find largest even (for loop unrolling) strip lenght that fits in L1 cache 
    for (STRIP = 88; STRIP>0; STRIP-=2){
        if (sub_dim%STRIP == 0){
            break;
        }
    }
	...
	*/
    ...
    for (int r=0; r<p_dim; r++ ){
        ...
        if (sub_dim < 32){
            // with loop unrolling
            for (int i = 0; i<sub_dim; i+=2){
                for (int j = 0; j<sub_dim; j+=2){
                    for (int k = 0; k<sub_dim; k++){
                        loc_C[I_SMAT(i,j)] += curr_A[I_SMAT(i,k)]*loc_B[I_SMAT(k,j)];
                        loc_C[I_SMAT(i+1,j)] += curr_A[I_SMAT(i+1,k)]*loc_B[I_SMAT(k,j)];
                        loc_C[I_SMAT(i,j+1)] += curr_A[I_SMAT(i,k)]*loc_B[I_SMAT(k,j+1)];
                        loc_C[I_SMAT(i+1,j+1)] += curr_A[I_SMAT(i+1,k)]*loc_B[I_SMAT(k,j+1)];
                    }
                }
            }
        // with loop unrolling and cache blocking for L1
        } else {
            for (int ii = 0; ii < sub_dim; ii += STRIP){
                for (int kk =0; kk < sub_dim; kk += STRIP){
                    for (int j = 0; j<sub_dim; j+=2){
                        for (int i = ii; i < ii + STRIP; i+=2){
